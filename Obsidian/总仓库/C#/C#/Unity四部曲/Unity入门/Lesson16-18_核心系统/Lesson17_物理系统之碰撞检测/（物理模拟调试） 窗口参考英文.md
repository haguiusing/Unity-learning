...
|**属性**|**描述**|
|---|---|
|**Number of items per row**|Define how many selected GameObjects to display horizontally in the top row of the **Info** tab.  <br>  <br>By default, if you select or lock multiple GameObjects, Unity displays them all vertically. However, you can arrange them horizontally instead. Use **Number of items per row** to define how many GameObjects to display next to each other on one horizontal row, up to a maximum of 10.  <br>  <br>If the number of selected or locked GameObjects exceeds this number, the more recently selected GameObjects appear on a new row underneath the first.  <br>  <br>GameObjects always appear in the **Info** tab in the order of selection. The default value for this property is 1.|
...
The **Info** tab displays the following properties when you select a GameObject in the Hierarchy that has a physics body (that is, a [Rigidbody](https://docs.unity.cn/cn/2022.2/Manual/class-Rigidbody.html) or an [ArticulationBody](https://docs.unity.cn/cn/2022.2/Manual/class-ArticulationBody.html)).

|**属性**|**描述**|
|---|---|
|**游戏对象**|Displays the name of the selected or locked GameObject that the following properties refer to.|
...
|**Sleep Threshold**|The energy threshold for the physics body, below which the physics engine stops including the body in physics updates. By default, this is the **Sleep Threshold** value defined in the [global Physics settings](https://docs.unity.cn/cn/2022.2/Manual/class-PhysicsManager.html).|
...
|**属性**|**描述**|
|---|---|
|**Show Physics Scene**|Select which Physics Scenes to include in the visualization, if you have [multiple physics scenes](https://docs.unity.cn/cn/2022.2/Manual/physics-multi-scene.html) loaded at once.|
|**Show Unity Scene**|Select which Unity Scenes to include in the visualization, if you have [multiple scenes](https://docs.unity.cn/cn/2022.2/Manual/MultiSceneEditing.html) loaded at once.|
|**Show Layers**|Select which [Layers](https://docs.unity.cn/cn/2022.2/Manual/Layers.html) to display in the visualization.|
...
|**Show Rigidbodies**|Enable **Show Rigidbodies** to display visualization for [Rigidbody](https://docs.unity.cn/cn/2022.2/Manual/class-Rigidbody.html) components. Disable **Show Rigidbodies** to remove visualization for Rigidbody components.|
...
|**Show Articulation Bodies**|Enable **Show Articulation Bodies** to display visualization for [ArticulationBody](https://docs.unity.cn/cn/2022.2/Manual/class-ArticulationBody.html) components. Disable **Show Articulation Bodies** to remove visualization for ArticulationBody components.|
...
|- **Show BoxColliders**|Enable **Show BoxColliders** to display visualization for [Box Colliders](https://docs.unity.cn/cn/2022.2/Manual/class-BoxCollider.html). Disable **Show BoxColliders** to remove visualization for Box Colliders.|
|- **Show SphereColliders**|Enable **Show SphereColliders** to display visualization for [Sphere Colliders](https://docs.unity.cn/cn/2022.2/Manual/class-SphereCollider.html). Disable **Show SphereColliders** to remove visualization for Sphere Colliders.|
|- **Show CapsuleColliders**|Enable **Show CapsuleColliders** to display visualization for [Capsule Colliders](https://docs.unity.cn/cn/2022.2/Manual/class-CapsuleCollider.html). Disable **Show CapsuleColliders** to remove visualization for Capsule Colliders.|
|- **Show MeshColliders (convex)**|Enable **Show MeshColliders (convex)** to display visualization for [Mesh Colliders](https://docs.unity.cn/cn/2022.2/Manual/class-MeshCollider.html) that have their **Is Convex** property enabled. Disable **Show MeshColliders (convex)** to remove visualization for Mesh Colliders that have their **Is Convex** property enabled.|
...
|- **Show TerrainColliders**|Enable **Show TerrainColliders** to display visualization for [Terrain Colliders](https://docs.unity.cn/cn/2022.2/Manual/class-TerrainCollider.html). Disable **Show TerrainColliders** to remove visualization for Terrain Colliders.|
...
|**Rigidbodies**|Select a color to represent [Rigidbody](https://docs.unity.cn/cn/2022.2/Manual/class-Rigidbody.html) components in the visualization.|
...
|**Articulation Bodies**|Select a color to represent [Articulation Body components](https://docs.unity.cn/cn/2022.2/Manual/class-ArticulationBody.html) in the visualization.|
...
|**Terrain Tiles Max**|Set the maximum number of [Terrain](https://docs.unity.cn/cn/2022.2/Manual/script-Terrain.html) tiles in the visualization. This is set to 60 by default.|
...
|- **Show All Contacts**|Enable **Show All Contacts** to display visualization for all collision contact data in the Physics Debug visualization. Disable **Show All Contacts** to limit the contact data that appears in the Physics Debug visualization.  <br>When **Show All Contacts** is disabled, only the following contacts appear in the visualization:  <br>- Colliders with `MonoBehaviour` scripts that receive messages from `OnCollisionEnter`, `OnCollisionSay`, or `OnCollisionExit`  <br>- Colliders that have the `providesContact` property set to true.|
