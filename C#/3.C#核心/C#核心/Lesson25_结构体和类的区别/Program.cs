namespace Lesson25_结构体和类的区别
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            //因此他们门的存储位置一个在栈上，一个在堆上
            //通过之前知识点的学习，我相信你能够从此处看出他们在使用的区别一值和引用对象在赋值时的区别。

            //结构体和类在使用上很类似，结构体至可以用面向对象的思想来形容一类对象
            //结构体具备着面向对像象思想中封装的特性，但是它不具备继承和多态的特性，因此大大减少了它的使用频率
            ...
            //5.结构体不能申明无参的构造函数，而类可以(C#9.0后可以)
            //6.结构体申明有参构造函数后，无参构造不会被顶掉
            //7.结构体不能申明析构函数，而类可以
            //8.结构体不能被继承，而类可以
            //9.结构体需要在构造函数中初始化所有成员变量，而类随意
            //10.结构体本身不能被静态static修饰（不存在静态结构体，内部可以有静态成员），而类可以
